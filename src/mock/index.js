...
if (taskStorage.length === 0) {
  taskStorage = [
    ...
    },
    {
      id: 2,
      name: "训练平台UI界面优化",
      description: "优化训练平台的用户界面，提升用户体验和视觉效果。包含页面布局调整、色彩搭配优化、交互体验改进等。",
      status: "未开始",
      difficulty: "中",
      assignee: "何也",
      collaborators: ["唐硕"],
    ...
    },
    {
      id: 3,
      name: "模拟交易功能开发",
      description: "开发模拟交易核心功能，包含下单、撤单、持仓管理等基础交易功能。需要确保数据准确性和系统稳定性。",
      status: "即将完成",
      difficulty: "高",
      assignee: "黄骏雄",
...
Mock.mock(/\/api\/tasks$/, "get", (options) => {
  ...
  return createResponse(true, [...taskStorage], "获取任务列表成功")
})
...
Mock.mock(/\/api\/tasks\/(\d+)$/, "put", (options) => {
  ...
  if (updateData.progress !== undefined && updateData.progress !== oldTask.progress) {
    const newProgressRecord = {
      ...
      action: "更新进度",
      description: updateData.progressDescription || `进度从 ${oldTask.progress}% 更新至 ${updateData.progress}%`,
...
Mock.mock(/\/api\/tasks\/batch$/, "delete", (options) => {
  ...
  return createResponse(
    ...
    `成功删除 ${deletedTasks.length} 个任务${notFoundIds.length > 0 ? `，${notFoundIds.length} 个任务不存在` : ""}`,
  )
})
...
Mock.mock(/\/api\/tasks\/(\d+)\/status$/, "patch", (options) => {
  ...
  taskStorage[taskIndex].progressHistory = [...(taskStorage[taskIndex].progressHistory || []), statusHistory]
...
// 更新任务进度
Mock.mock(/\/api\/tasks\/(\d+)\/progress$/, "patch", (options) => {
  ...
  taskStorage[taskIndex].progressHistory = [...(taskStorage[taskIndex].progressHistory || []), progressHistory]
...
Mock.mock(/\/api\/tasks\/(\d+)\/comments$/, "post", (options) => {
  ...
  const commentHistory = {
    ...
    user: commentData.user || "匿名用户",
    action: "添加评论",
    description: `添加了新评论：${commentData.content.substring(0, 50)}${commentData.content.length > 50 ? "..." : ""}`,
    type: "comment",
  }
...
Mock.mock(/\/api\/tasks\/filter$/, "post", (options) => {
  ...
  if (filters.projectParentId && filters.projectParentId !== "all") {
    filteredTasks = filteredTasks.filter((task) => task.projectParentId === filters.projectParentId)
  }
  ...
  if (filters.startDate) {
    filteredTasks = filteredTasks.filter((task) => new Date(task.startDate) >= new Date(filters.startDate))
  }

  if (filters.endDate) {
    filteredTasks = filteredTasks.filter((task) => new Date(task.dueDate) <= new Date(filters.endDate))
  }
...
Mock.mock(/\/api\/tasks\/stats$/, "get", (options) => {
  ...
  taskStorage.forEach((task) => {
    ...
    // 按项目类型统计
    stats.byProjectType[task.projectTypeName] = (stats.byProjectType[task.projectTypeName] || 0) + 1
